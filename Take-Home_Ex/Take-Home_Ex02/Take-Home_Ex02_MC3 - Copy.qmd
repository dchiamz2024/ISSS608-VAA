...
# Title rendered using markdown above widget
cat("### Interactive Network of Communication Between People and Vessels")

visNetwork(
  nodes = comm_node_df %>% rename(id = id),
  edges = comm_vis_edges
) %>%
  visOptions(highlightNearest = TRUE, nodesIdSelection = TRUE) %>%
  visGroups(groupname = "Person", color = "#fc8d62", shape = "dot") %>%
  visGroups(groupname = "Vessel", color = "#66c2a2", shape = "triangle") %>%
  visLegend() %>%
...
# Step 1: Summarize communication volume between pairs
comm_edges_vis <- comm_links_filtered %>%
  count(sender, receiver, sort = TRUE) %>%
  rename(from = sender, to = receiver, value = n)
...
# Step 4: Compute degree centrality and assign hierarchy levels
# Convert to igraph
graph_ig <- graph_from_data_frame(comm_edges_vis, directed = TRUE, vertices = nodes_vis)

# Compute total degree (in + out)
degree_vals <- degree(graph_ig, mode = "all")

# Assign layout level: higher degree = inner layer
nodes_vis <- nodes_vis %>%
  left_join(tibble(id = names(degree_vals), degree = degree_vals), by = "id") %>%
  mutate(level = max(degree, na.rm = TRUE) - degree)
...
# Step 6: Define legend
legend_nodes <- list(
  list(label = "Person", shape = "dot", color = "#fc8d62"),
  list(label = "Vessel", shape = "triangle", color = "#66c2a2")
)

# Step 7: Render visNetwork with hierarchical layout (degree-based levels)
cat("### Degree-Based Communication Network (Concentric Layout by Influence)")
...
visNetwork(nodes_vis, edges_vis, width = "100%", height = "900px") %>%
  ...
  visHierarchicalLayout(
    enabled = TRUE,
    direction = "UD",  # Use "UD" for concentric feel
    levelSeparation = 50,
    nodeSpacing = 800
  ) %>%
  visLayout(randomSeed = 1818)
...
```{r}
# Render both graphs side by side in Quarto
htmltools::tagList(
  htmltools::tags$div(
    style = "display: flex; justify-content: space-between;",
    htmltools::tags$div(style = "width: 49%;", louvain_net),
    htmltools::tags$div(style = "width: 49%;", walktrap_net)
  )
)
