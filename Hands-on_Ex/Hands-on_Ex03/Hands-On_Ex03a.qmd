...
**Step 3: Plot Visuals all into single graph (Interactive)**
...
```
From here, we note that the readability is not as good, hence I would plot them as individual graphs as below. 
...
# Function to build styled CPI plot
make_cpi_plot <- function(category_name) {
  ggplot(cpi_long %>% filter(Item == category_name),
         aes(x = Year, y = CPI,
             tooltip = paste("Year:", Year, "<br>CPI:", round(CPI, 1)),
             data_id = Year)) +
...
# Generate wide interactive plots (stretch width + reasonable height)
p10 <- girafe(ggobj = make_cpi_plot("Food"), width = "100%", width_svg = 14, height_svg = 5)
p20 <- girafe(ggobj = make_cpi_plot("Health"), width = "100%", width_svg = 14, height_svg = 5)
p30 <- girafe(ggobj = make_cpi_plot("Transport"), width = "100%", width_svg = 14, height_svg = 5)
p40 <- girafe(ggobj = make_cpi_plot("Education"), width = "100%", width_svg = 14, height_svg = 5)

# Stack them in full-width layout
htmltools::tags$div(
