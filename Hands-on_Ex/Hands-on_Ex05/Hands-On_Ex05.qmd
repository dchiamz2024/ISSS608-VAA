...
![](/Images/Hands-on_Ex5_Image01.jpg){fig-align="center"}

### **9.3.2 The nodes data**

-   *GAStech_email_nodes.csv* which consist of the names, department and title of the 55 employees.

![](/Images/Hands-on_Ex5_Image02.jpg){fig-align="center"}
...
GAStech_edges <- read_csv("DataEx05/GAStech_email_edge-v2.csv")
```
...
-   *activate()* verb from **tidygraph** serves as a switch between tibbles for nodes and edges. All dplyr verbs applied to **tbl_graph** object are applied to the active tibble.
    ...
    ![](/Images/Hands-on_Ex5_Image03.jpg)
...
![](/Images/Hands-on_Ex5_Image04.jpg)

![](/Images/Hands-on_Ex5_Image05.jpg)
...
## **9.9 Self Hands-on Practice on the codes in this section**

![](/Images/Hands-on-Practice.png){width="558"}

I will be using the previously loaded csv files in `GAStech_nodes` and `GAStech_edges`

### 9.9.1 Trying Out Plots

#### Step 1a: Check Column Names
...
colnames(GAStech_edges)

```

#### Step 1b: Rename `source` to `target`

```{r}

# Create a weighted edge list by counting source-target pairs
GAStech_edges_summary <- GAStech_edges %>%
  count(source, target, name = "weight") %>%
  rename(from = source, to = target)

```

#### Step 2: Calculate Degree and Size Nodes

```{r}

# Compute degree using igraph

g <- graph_from_data_frame(GAStech_edges, 
                           directed = TRUE, 
                           vertices = GAStech_nodes)
GAStech_nodes$size <- degree(g, mode = "all")


```

#### Step 3: Basic Interactive Network with visNetwork

```{r}

visNetwork(GAStech_nodes, GAStech_edges) %>%
  visOptions(highlightNearest = TRUE, 
             nodesIdSelection = TRUE) %>%
  visLayout(randomSeed = 123)
...
#### Step 4: Add Group Colors and Filtering

```{r}

visNetwork(GAStech_nodes, GAStech_edges) %>%
  visGroups(groupname = "Administration", color = "lightblue") %>%
  visGroups(groupname = "IT", color = "orange") %>%
...
#### Step 5: Filter Edges by Email Volume (e.g., weight ≥ 10)

```{r}

# Filter for high-volume communication
GAStech_edges_strong <- GAStech_edges_summary %>% filter(weight >= 10)

# Plot with visNetwork
visNetwork(GAStech_nodes, GAStech_edges_strong) %>%
...
### 9.9.2 Trying Out Graph Layouts with `ggraph`
...
email_graph <- tbl_graph(nodes = GAStech_nodes, edges = GAStech_edges_summary, directed = TRUE)

# Circle Layout
ggraph(email_graph, layout = "circle") +
...
### 9.9.3 Trying Out Network Node & Edge Metrics
...
# View top centrality nodes
email_graph %>%
  as_tibble() %>%
  arrange(desc(degree)) %>%
  select(label, group, degree, betweenness, closeness) %>%
  head(10)

```

### 9.9.3 Trying Out `facet_edges()` or `facet_nodes()` for Multiple Views

```{r fig.height=8, fig.width=12, warning=FALSE}

# Example: Facet edges by weekday (requires joining original edge data with time info)
GAStech_edges_summary <- GAStech_edges %>%
  count(source, target, Weekday, name = "weight") %>%
  rename(from = source, to = target)

facet_graph <- tbl_graph(nodes = GAStech_nodes, edges = GAStech_edges_summary, directed = TRUE)

ggraph(facet_graph, layout = "fr") +
  geom_edge_link(aes(alpha = weight), show.legend = FALSE) +
...
# --- Load Data ---
GAStech_nodes <- read_csv("DataEx05/GAStech_email_node.csv")
raw_edges <- read_csv("DataEx05/GAStech_email_edge.csv")

# --- Prepare Nodes ---
# Assume id and label columns exist and use Department as group
GAStech_nodes <- GAStech_nodes %>%
  rename(id = id, label = label) %>%
  mutate(group = Department)

# --- Prepare Edges with subject grouping ---
subject_edges <- raw_edges %>%
  count(source, target, MainSubject, name = "weight") %>%
  rename(from = source, to = target)

# --- Create Interactive Network ---
visNetwork(GAStech_nodes, subject_edges) %>%
  visOptions(
    highlightNearest = TRUE,
    nodesIdSelection = TRUE,
    selectedBy = list(variable = "MainSubject", multiple = FALSE)
  ) %>%
  visGroups(groupname = "Administration", color = "lightblue") %>%
  visGroups(groupname = "Engineering", color = "orange") %>%
  visGroups(groupname = "IT", color = "lightgreen") %>%
  visLayout(randomSeed = 42)
```
