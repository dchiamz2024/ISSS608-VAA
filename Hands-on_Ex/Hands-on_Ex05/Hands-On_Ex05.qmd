...
### 9.9.3a Trying Out Network Node & Edge Metrics
...
### 9.9.3b Trying Out `facet_edges()` or `facet_nodes()` for Multiple Views
...
# --- Visualise by Weekday with department coloring ---
ggraph(facet_graph, layout = "lgl") +
  geom_edge_link(aes(alpha = weight), show.legend = FALSE) +
  geom_node_point(aes(color = group), size = 3) +  # Use `group`, not Department
  geom_node_text(aes(label = label), size = 2, repel = TRUE) +
  facet_edges(~Weekday) +
  theme_graph() +
  labs(title = "GAStech Email Networks by Weekday (Grouped by Department)")
