...
covid19 <- read_csv("DataEx04/COVID-19_DKI_Jakarta.csv") %>%
...
**This is an attempt to plot the funnel plot using the parameters above.**
...
funnel_plot(
  .data = covid19,
  numerator = Death,            # events of interest
  denominator = Positive,       # total population (cases)
  group = `Sub-district`,       # grouping
  limit = 95,                   # confidence limit
  label_outliers = TRUE,        # label outliers
  Poisson_limits = TRUE,        # add Poisson control limits
  OD_adjust = TRUE,             # overdispersion-adjusted limits
  xrange = c(0, 6500),          # zoom in on x-axis
  yrange = c(0, 0.05),          # zoom in on y-axis
  title = "COVID-19 Fatality Funnel Plot",
  x_label = "Cumulative Positive COVID-19 Cases",
...
funnel_plot(
  ...
  xrange = c(0, 6500),  #<<
  yrange = c(0, 0.05),   #<<
  title = "COVID-19 Fatality Funnel Plot by Sub-district"
)
